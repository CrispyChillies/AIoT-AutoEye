...
def create_user():
    try:
        ...
        if not personal.get('name') or not personal.get('email'):
            return jsonify({"error": "name and email are required in personal object"}), 400
        ...
        return jsonify({"error": f"User with id '{data.get('_id')}' already exists"}), 409
...
def update_traffic_data(traffic_id):
    try:
        ...
        result = traffic_collection.update_one({"_id": traffic_id}, {"$set": update_doc})
        if result.matched_count:
